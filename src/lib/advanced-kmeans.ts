export interface AdvancedStudent {
  id: string;
  name: string;
  maths: number;
  programming: number;
...
export function getOverallSkillLevel(student: AdvancedStudent): SkillLevel {
  const avg = (student.maths + student.programming + student.communication) / 3;
  return getSkillLevel(avg);
}
...
function getFeatures(s: AdvancedStudent): number[] {
  return [s.maths, s.programming, s.communication];
}

function computeCentroid(students: AdvancedStudent[]): number[] {
...
export function advancedKMeans(students: AdvancedStudent[], k: number, maxIter = 50): AdvancedCluster[] {
  ...
  // Build clusters
  const clusters: AdvancedCluster[] = centroids.map((centroid, i) => ({
    centroid,
...
export function balancedGrouping(students: AdvancedStudent[], k: number): AdvancedCluster[] {
  // First run K-Means to identify skill clusters
  const skillClusters = advancedKMeans(students, Math.min(k, students.length));
  
  // Now distribute students from each skill cluster evenly across groups
  const groups: AdvancedStudent[][] = Array.from({ length: k }, () => []);
  
  // Sort students by total skill score within each cluster
  const allSorted: AdvancedStudent[] = [];
...
export const ADVANCED_SAMPLE_STUDENTS: AdvancedStudent[] = [
